...
- The Value Function `v(s)` estimates how "good" it is for an agent to be in a particular state. More formally, it's the expected return `G_t` given that the agent is in state `s` under some chosen policy `pi`. `v_pi(s) = Ex[G_t | S_t = s]`. Just to be clear, the value function is specific to a given policy `pi`.
...
- The optimal state value function `v*(s)` is the value function for the optimal policy `p*`. Note that there could exist more than one optimal policy. Same for `q*(s, a)`. The Bellman Optimality Equation defines how the optimal value of a state is related to the optimal value of successor states. It has a "max" instead of an average.
